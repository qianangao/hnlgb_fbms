...
const Table = ({
  ...
  const getEmployeeList = params =>
    new Promise(resolve => {
      dispatch({
        type: 'workDynamic/workDynamicList',
        payload: { ...params, status: publishStatus, type: 4 },
        resolve,
      });
    });
  // 删除
  const deleteReturnworkPerson = ids => {
    dispatch({
      type: 'workDynamic/deleteWorkDynamic',
      payload: {
        ids,
      },
    });
...
export default connect(({ workDynamic, global }) => ({
  workDynamic,
  enums: global.enums,
}))(Table);
