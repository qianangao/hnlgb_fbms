...
const MedicalPolicyFrom = ({ form, id, dispatch, loading }) => {
  useEffect(() => {
    if (id) {
      new Promise(resolve => {
        dispatch({
          type: 'medicalPolicy/detailMedicalPolicyInfo',
          payload: { id },
          resolve,
        });
      }).then(data => {
  ...
  const formItems = [
    {
      ...
      rules: [
        ...
        { max: 128, message: '医疗政策名称请小于128位!', whitespace: true },
      ],
    },
    {
      label: '内容',
...
MedicalPolicyFrom.useForm = AdvancedForm.useForm;
