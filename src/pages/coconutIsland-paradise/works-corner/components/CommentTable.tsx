...
const CommentTable = ({ worksCorner, dispatch, id, enums }) => {
  const { commentTableRef } = worksCorner;
  const columns = [
    {
      title: '序号',
      dataIndex: 'index',
  ...
  const getCommentList = params =>
    new Promise(resolve => {
      dispatch({
        type: 'worksCorner/getCommentList',
  ...
  const deleteComment = ids => {
    dispatch({
      type: 'worksCorner/deleteComment',
      payload: {
        ids,
      },
    });
