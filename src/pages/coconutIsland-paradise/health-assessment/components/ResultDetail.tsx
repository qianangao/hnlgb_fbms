...
const ResultDetail = ({ id, dispatch, detailResultData, enums }) => {
  const dictAssessGradeEnums = enums.dictAssessGrade;
  useEffect(() => {
    dispatch({
  ...
  return (
    <>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={8}>
          <ResultCard title="测评分数" blockData={detailResultData.fraction} />
        </Col>
        <Col span={8}>
          <ResultCard
            title="测评等级"
            blockData={dictAssessGradeEnums[detailResultData.dictAssessGrade]}
          />
        </Col>
        <Col span={8}>
          <ResultCard title="测评时间" blockData={detailResultData.assessTime} />
        </Col>
      </Row>
      {detailResultData.healthAssessToVo &&
        detailResultData.healthAssessToVo.map(item => (
          <Descriptions size="middle" column={1} style={{ marginBottom: 16 }}>
            ...
            >{`题目:${item.subject}`}</Descriptions.Item>
            ...
            <Descriptions.Item label="您的选择是" style={{ backgroundColor: '#f5f5f5' }}>
              {item.answer}
            </Descriptions.Item>
          </Descriptions>
        ))}
